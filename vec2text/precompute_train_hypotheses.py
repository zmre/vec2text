...
def precompute(start_idx: int, num_samples: int):
    out_path = os.path.join(
        DATASET_CACHE_PATH,
        "/inversion/msmarco_msl128_hypotheses/",
        f"msmarco_{num_samples}_{start_idx}.arrow",
    )
...
def gather():
    ...
    files = sorted(
        glob.glob(
            f"{DATASET_CACHE_PATH}/inversion/msmarco_msl128_hypotheses/msmarco_{n_samples}_*"
        )
    )
    gathered_dataset_path = (
        f"{DATASET_CACHE_PATH}/inversion/msmarco_msl128_hypotheses/msmarco_full.cache"
    )
    datasets_list = []
